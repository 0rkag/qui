...
var (
	...
	ErrInvalidFileExistsAction = errors.New("invalid file exists action (must be abort, skip, or overwrite)")
	ErrInvalidSourceAction    = errors.New("invalid source action (must be keep, pause, or delete)")
...
type TransferRequest struct {
	...
	FileExistsAction models.FileExistsAction `json:"fileExistsAction"` // What to do if target file exists (abort/skip/overwrite)
	SourceAction     models.SourceAction     `json:"sourceAction"`     // What to do with source after transfer (keep/pause/delete)
...
func (r *TransferRequest) Validate() error {
	...
	// Validate enum fields if provided (empty string means use default)
	if r.FileExistsAction != "" && !r.FileExistsAction.IsValid() {
		return ErrInvalidFileExistsAction
	}
	if r.SourceAction != "" && !r.SourceAction.IsValid() {
		return ErrInvalidSourceAction
	}
	return nil
}
...
type MoveRequest struct {
	...
	PathMappings     map[string]string       `json:"pathMappings,omitempty"`
	FileExistsAction models.FileExistsAction `json:"fileExistsAction"`
	SourceAction     models.SourceAction     `json:"sourceAction"`
